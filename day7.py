...
def smallest_to_delete(fileName):
    lines = read_data(fileName)
    directoryDictionary = command_handler(lines)
    minAbove = sys.maxsize
    print(directoryDictionary['/'])
    if (TOTAL_SPACE - directoryDictionary['/']) > SPACE_NEEDED:
        print("There's already enough space.")
    else:
        for size in directoryDictionary.values():
            if size + TOTAL_SPACE - directoryDictionary['/'] > SPACE_NEEDED and size < minAbove:
                minAbove = size
        print("The smallest that can be deleted to free enough space has size " + str(minAbove))



sum_directory_sizes("day7.txt")
smallest_to_delete("day7.txt")